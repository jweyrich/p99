...
#define O_RWL_CALLBACK_PAIR(T) P99_PASTE3(o_rwl_, T, _orwl_wh_t)
...
typedef struct {                                                                  \
 ...
 DOCUMENT_INIT(O_RWL_CALLBACK_PAIR(T))                                            \
inline                                                                            \
O_RWL_CALLBACK_PAIR(T) *                                                          \
P99_PASTE2(O_RWL_CALLBACK_PAIR(T), _init)(O_RWL_CALLBACK_PAIR(T) *arg,            \
                                   T* Arg,                                        \
                                   orwl_wh *Wh,                                   \
                                   orwl_wq *Wq                                    \
                                   ) {                                            \
 ...
 DOCUMENT_DESTROY(O_RWL_CALLBACK_PAIR(T))                                         \
inline                                                                            \
 void P99_PASTE2(O_RWL_CALLBACK_PAIR(T), _destroy)(O_RWL_CALLBACK_PAIR(T) *arg) { \
  if (!arg) return;                                                               \
...
P99_INSTANTIATE(                                                                    \
                O_RWL_CALLBACK_PAIR(T)*,                                            \
                P99_PASTE2(O_RWL_CALLBACK_PAIR(T), _init),                          \
                O_RWL_CALLBACK_PAIR(T)*,                                            \
                T*,                                                                 \
                orwl_wh*,                                                           \
                orwl_wq*                                                            \
  ...
  if (orwl_wh_valid(wh)) {                                                          \
    ...
    if (wq) {                                                                       \
      ...
      if (wq->head == wh) {                                                         \
        ...
        O_RWL_CALLBACK_PAIR(T) *pair                                                \
          = P99_NEW(O_RWL_CALLBACK_PAIR(T), arg, wh, wq);                           \
        ...
        ret = P99_PASTE2(O_RWL_CALLBACK_PAIR(T), _create)(pair, P99_0(pthread_t*)); \
        ...
        /* generated.                                                */             \
        /*o_rwl_ ## T ## _orwl_wh_t ## _delete(pair);*/                             \
        pthread_mutex_unlock(&wq->mut);                                             \
      }                                                                             \
    }                                                                               \
  }                                                                                 \
