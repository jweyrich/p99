...
P99_PROTOTYPE(worker_thread*, worker_thread_init, worker_thread *, orwl_barrier*, size_t, size_t, size_t, orwl_mirror*, char*);
...
int main(int argc, char **argv) {
  ...
  orwl_mirror*const mirror_back = orwl_mirror_vnew(number + 2);
  orwl_mirror* mirror = mirror_back + 1;
  ...
  pthread_t *const id = pthread_t_vnew(number/2);
    ...
    P99_UNWIND_PROTECT {
...

      {
        ...
        for (ssize_t i = -1; i <= (ssize_t)number; ++i) {
          ...
          orwl_mirror_connect(&mirror[i], there, ORWL_KEY(basic, gpos));
        ...
        if (thread%2) {
          ...
          worker_thread_create_joinable(
                                        worker_thread_init(&arg[thread2],
                                                           ...
                                                           thread + offset,
                                                           phases,
                                                           /* give it the starting position of the location on
                                                            * the left. */
                                                           &mirror[thread - 1],
  ...
  P99_PROTECT:
    ...
    orwl_mirror_vdelete(mirror_back);
    seed_get_clear();
  }
  return EXIT_SUCCESS;
}
